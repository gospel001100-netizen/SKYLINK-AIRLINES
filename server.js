// server.js - SKYLINK FINAL COMPLETE - WHOLE 150+ COUNTRIES + FIXED CALLBACK + REAL PAYSTACK
...
// WHOLE COUNTRIES - 150+ WORLDWIDE - YEMEN, SAUDI, UAE, QATAR, NIGERIA, GHANA, ALL
const AIRPORTS = [
...
function payWithPaystack(){
  ...
  if(typeof PaystackPop === 'undefined'){
    alert("Paystack not loaded. Check internet or ad blocker.");
  ...
  try{
    var handler = PaystackPop.setup({
      ...
      .then(function(d){
          if(d.boardingUrl){ window.location = d.boardingUrl; }
          else { alert(d.error||"Booking failed"); btn.innerText="Pay NGN 2,150 with Paystack & Generate Boarding Pass"; btn.disabled=false; }
        })
      .catch(function(err){ alert("Error: "+err.message); btn.innerText="Pay NGN 2,150 with Paystack & Generate Boarding Pass"; btn.disabled=false; });
      }
    });
    handler.openIframe();
  }catch(err){
...
app.get('/skylink-admin-gospel-2024', async (req,res)=>{
  ...
  res.send('<html><body><h2>Admin - '+all.length+' - Total NGN '+(all.length*2150)+' Real Money</h2><table border=1><tr><th>BOOKING</th><th>TRACKING</th><th>NAME</th><th>ROUTE</th><th>AMOUNT</th><th>REF</th></tr>'+rows+'</table></body></html>');
});
...
app.get('/boarding-pass', async (req,res)=>{
  ...
  res.send('<html><body><div style="max-width:800px;margin:auto;background:#fff;border:2px solid #000"><div style="padding:18px;border-bottom:3px solid #000"><b>SKYLINK AIRLINES - PAID NGN 2,150</b></div><div style="padding:20px"><div>'+b.name+'</div><div>'+b.fromFull+' to '+b.toFull+'</div><div>Flight '+b.flight+' Seat '+b.seat+' Gate '+b.gate+'</div><img src="'+qr+'" style="width:120px"><div>Track: https://'+req.get('host')+'/track?code='+b.tracking+' Ref: '+b.paystackRef+'</div></div></div></body></html>');
});
...
app.get('/track', async (req,res)=>{
  ...
  res.send('<html><head><meta name="viewport" content="width=device-width,initial-scale=1"><style>body{margin:0;background:#f1f5f9;display:flex;justify-content:center;padding:12px;font-family:Arial}.card{width:100%;max-width:560px;background:#fff;border-radius:18px;border:2px solid #000;overflow:hidden}.head{background:#000;color:#fff;padding:22px;text-align:center}.body{padding:22px}</style></head><body><div class="card"><div class="head"><h1>FLIGHT STATUS - PAID</h1><p>'+b.from+' -> '+b.to+'</p></div><div class="body"><b>Booking:</b> '+b.booking+'<br><b>Tracking:</b> '+b.tracking+'<br><b>Passenger:</b> '+b.name+'<br><b>Payment:</b> '+b.paystackRef+' - NGN 2,150</div></div></body></html>');
});

app.get('/health',(req,res)=> res.send('OK'));
app.listen(PORT, ()=> console.log('SKYLINK FINAL COMPLETE - WHOLE COUNTRIES + CALLBACK FIXED + REAL PAY'));
