// server.js - SKYLINK AIRLINES - REAL & STANDARD VERSION
// No permanent words for customers - Paystack ₦2,150 - Hidden Admin - Real Boarding Pass
  ...
  if(uri && mongoose){
    try{
      ...
      const schema = new mongoose.Schema({
        ...
        paystackRef: String, amount: Number,
        createdAt: String
      }, { _id: false });
      ...
      console.log(`Loaded ${all.length} bookings from cloud - Never deletes`);
      return;
    }catch(e){ console.log('DB fail, using file', e.message); }
  }
  try{
...
// HOME - CLEAN, NO PERMANENT, NO ADMIN LINK
app.get('/', (req,res)=>{
...
<div class="nav"><div class="logo"><span>SKYLINK</span><span>AIRLINES</span></div><div style="font-size:10px">IATA CERTIFIED</div></div>
<div class="hero"><h2 style="font-weight:900">Book Flight</h2><div class="card"><form onsubmit="payAndBook(event)"><label>Passenger Name</label><input id="pname" required><div style="display:flex;gap:12px"><div style="flex:1" class="rel"><label>FROM</label><input id="from" autocomplete="off" oninput="autoSuggest('from')" placeholder="LOS - Lagos" required><div id="from-suggest" class="suggest"></div></div><div style="flex:1" class="rel"><label>TO</label><input id="to" autocomplete="off" oninput="autoSuggest('to')" placeholder="JFK - New York" required><div id="to-suggest" class="suggest"></div></div></div><div style="display:flex;gap:12px"><div style="flex:1"><label>Departure Date & Time (Your Local Airport Time)</label><input type="datetime-local" id="depart" required></div><div style="flex:1"><label>Class</label><select id="class"><option>ECONOMY</option><option>BUSINESS</option></select></div></div><div class="warning">⚠️ WARNING!!! Transfer this exact amount: <b>₦2,150</b> - Do not pay more or less to avoid booking failure. Amount is fixed for confirmation.</div><button class="btn" id="payBtn">Pay ₦2,150 with Paystack & Generate Boarding Pass</button></form></div><p style="text-align:center;margin-top:16px;font-size:11px;color:#64748b">Secure payment by Paystack • Instant boarding pass • Trackable worldwide</p></div>
<script>
const airports=${JSON.stringify(AIRPORTS)};
const PAYSTACK_PUBLIC_KEY="${process.env.PAYSTACK_PUBLIC_KEY || 'pk_test_xxxxxxxxxxxxxxxxxxxxxxxx'}";
...
async function payAndBook(e){
  ...
  const payload={name:document.getElementById('pname').value,from,to,depart:document.getElementById('depart').value,class:document.getElementById('class').value};
  ...
  let handler = PaystackPop.setup({
    ...
    onClose: function(){ btn.innerText='Pay ₦2,150 with Paystack & Generate Boarding Pass'; btn.disabled=false; alert('Payment cancelled'); },
    callback: async function(response){
      payload.paystackRef=response.reference;
      const r=await fetch('/api/book',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});
      const d=await r.json();
      if(d.boardingUrl){ window.location=d.boardingUrl; } else { alert(d.error||'Booking failed'); btn.innerText='Pay ₦2,150 with Paystack & Generate Boarding Pass'; btn.disabled=false; }
    }
  });
  handler.openIframe();
}
...
app.post('/api/book', async (req,res)=>{
  ...
  // PAYSTACK VERIFICATION - NO FREE BOOKING
  if(!paystackRef){ return res.status(400).json({error:'Payment required - Pay ₦2,150 first'}); }
  if(process.env.PAYSTACK_SECRET_KEY && axios){
    try{
      ...
      if(verify.data.data.amount < 215000){ return res.status(400).json({error:'Invalid amount - Pay exact ₦2,150'}); }
    }catch(e){ console.log('Paystack verify failed, but allowing for test mode', e.message); }
  }
...
// HIDDEN ADMIN - ONLY YOU KNOW THIS LINK
app.get('/skylink-admin-gospel-2024', async (req,res)=>{
  ...
  const rows=all.map(b=>\`<tr style="border-bottom:1px solid #e2e8f0"><td style="padding:10px;font-weight:800">\${b.booking}</td><td style="padding:10px;font-weight:800">\${b.tracking}</td><td style="padding:10px">\${b.name}</td><td style="padding:10px">\${b.from}->\${b.to}</td><td style="padding:10px">\${new Date(b.departISO).toLocaleString()}</td><td style="padding:10px">₦\${b.amount||2150}</td><td style="padding:10px"><a href="/boarding-pass?code=\${b.tracking}" style="background:#000;color:#fff;padding:6px 10px;border-radius:6px;text-decoration:none;font-size:11px">Pass</a> <a href="/track?code=\${b.tracking}" style="background:#22c55e;color:#fff;padding:6px 10px;border-radius:6px;text-decoration:none;font-size:11px">Track</a></td></tr>\`).join('');
  res.send(\`<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>SKYLINK Admin</title><style>body{margin:0;font-family:Arial;background:#f8fafc;padding:20px}.card{background:#fff;border-radius:16px;padding:20px;box-shadow:0 8px 30px rgba(0,0,0,.06);overflow:auto}table{width:100%;border-collapse:collapse;font-size:13px}th{padding:12px;text-align:left;background:#0f172a;color:#fff;font-size:11px}</style></head><body><h2>SKYLINK AIRLINES - Admin Dashboard</h2><p>Total Bookings: <b>\${all.length}</b> | Revenue: <b>₦\${all.length*2150}</b> | Storage: Cloud Never Deletes (Thousands safe)</p><div class="card"><table><tr><th>BOOKING</th><th>TRACKING</th><th>PASSENGER</th><th>ROUTE</th><th>DEPARTURE</th><th>AMOUNT</th><th>ACTION</th></tr>\${rows||'<tr><td colspan=7 style="padding:20px;text-align:center">No bookings yet</td></tr>'}</table></div><p><a href="/" style="background:#0f172a;color:#fff;padding:10px 16px;border-radius:10px;text-decoration:none">Home</a></p></body></html>\`);
});

// Block old admin routes
app.get('/admin', (req,res)=> res.redirect('/'));
app.get('/admin-panel', (req,res)=> res.redirect('/'));

// BOARDING PASS - REAL, BRIGHT, AUTHENTIC, NO PERMANENT WORD
app.get('/boarding-pass', async (req,res)=>{
  ...
  if(!b){return res.send('Booking not found - Invalid tracking code');}
...
app.listen(PORT, ()=> console.log(\`SKYLINK REAL running on \${PORT}\`));
