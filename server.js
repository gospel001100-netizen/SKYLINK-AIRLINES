// server.js - SKYLINK FINAL V3 - 100% REAL - DYNAMIC AIRCRAFT BY DISTANCE
...
app.get('/boarding-pass', async (req,res)=>{
  ...
  const aircraft = b.aircraft || "Boeing 737-800";
  const distance = b.distanceKm? b.distanceKm + " km" : "";
  ...
  res.send('<html><head><meta name="viewport" content="width=device-width,initial-scale=1"><title>Boarding Pass ' + b.booking + '</title><script src="https://cdnjs.cloudflare.com/ajax/libs/html2canvas/1.4.1/html2canvas.min.js"><\/script><style>*{box-sizing:border-box} body{margin:0;background:#e5e7eb;font-family:Arial,Helvetica,sans-serif;display:flex;flex-direction:column;align-items:center;padding:0}.ticket{width:100%;max-width:900px;background:#fff;margin:0;box-shadow:0 4px 20px rgba(0,0,0,.15);filter:brightness(1.15)}.header{background:#1a2b5e;color:#fff;padding:18px 24px;display:flex;justify-content:space-between;align-items:flex-start}.header-left h1{margin:0;font-size:28px;font-weight:900;letter-spacing:0.5px}.header-left h1 span{color:#facc15}.header-left.sub{font-size:10px;letter-spacing:1px;margin-top:4px;opacity:0.9}.header-right{font-size:11px;opacity:0.8;text-align:right}.content{padding:20px 24px;display:grid;grid-template-columns:1fr 200px;gap:20px}.label{font-size:10px;color:#6b7280;font-weight:700;letter-spacing:0.5px;text-transform:uppercase;margin-top:14px}.value{font-size:14px;font-weight:800;color:#111827;margin-top:2px;word-break:break-word}.big-name{font-size:18px;font-weight:900}.grid3{display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin-top:6px}.status-green{color:#16a34a;font-weight:900}.qr-box{border:1.5px solid #1a2b5e;padding:10px;text-align:center;background:#fff}.qr-box img{width:100%;max-width:180px;height:auto}.qr-link{font-size:8px;color:#1a2b5e;word-break:break-all;margin-top:6px;font-weight:600}.bottom-bar{background:#1a2b5e;color:#cbd5e1;padding:8px 24px;font-size:8px;text-align:center;letter-spacing:0.5px}.stub{padding:10px 24px;background:#fff;border-top:2px dashed #9ca3af;font-size:10px;font-weight:800}.btn-area{padding:14px;display:flex;gap:8px;justify-content:center;background:#f1f5f9;width:100%;max-width:900px}.btn-dl{background:#16a34a;color:#fff;border:none;padding:12px 18px;border-radius:8px;font-weight:900;cursor:pointer}.btn-tr{background:#0f2e6d;color:#fff;border:none;padding:12px 18px;border-radius:8px;font-weight:900;cursor:pointer}.btn-cp{background:#fff;color:#0f2e6d;border:1.5px solid #0f2e6d;padding:12px 18px;border-radius:8px;font-weight:900;cursor:pointer} @media(max-width:700px){.content{grid-template-columns:1fr}.qr-box{order:-1}}.no-capture{}</style></head><body><div class="ticket-wrap" id="ticketCapture"><div class="ticket"><div class="header"><div class="header-left"><h1>SKYLINK<span>AIRLINES</span></h1><div class="sub">IATA CERTIFIED • EST. 2018 • OFFICIAL BOARDING PASS</div></div><div class="header-right">' + host + '<br>' + b.tracking + '</div></div><div class="content"><div><div class="label">PASSENGER NAME / NOM DU PASSAGER</div><div class="value big-name">' + b.name + '</div><div class="label">FROM / DE</div><div class="value">' + b.fromFull + '</div><div class="label">TO / A</div><div class="value">' + b.toFull + '</div><div class="grid3"><div><div class="label">FLIGHT / VOL</div><div class="value">' + b.flight + '</div></div><div><div class="label">DATE</div><div class="value">' + new Date(b.departISO).toLocaleDateString('en-GB') + '</div></div><div><div class="label">SEAT / SIEGE</div><div class="value" style="font-size:18px">' + b.seat + '</div></div></div><div class="grid3"><div><div class="label">GATE / PORTE</div><div class="value">' + b.gate + '</div></div><div><div class="label">TERMINAL</div><div class="value">' + b.terminal + '</div></div><div><div class="label">CLASS</div><div class="value">' + b.class + '</div></div><div><div class="label">BAGGAGE</div><div class="value">' + b.baggage + '</div></div></div><div class="grid3" style="margin-top:12px"><div><div class="label">TRACKING CODE</div><div class="value">' + b.tracking + '</div></div><div><div class="label">STATUS</div><div class="value status-green">CONFIRMED / CONFIRME</div></div></div><div style="margin-top:16px"><div class="label">DEPARTURE / DEPART</div><div class="value">' + departStr + '</div><div class="label">ARRIVAL / ARRIVEE</div><div class="value">' + arriveStr + '</div></div><div style="margin-top:16px;font-size:11px;line-height:1.5"><b>AIRCRAFT:</b> ' + aircraft + ' (' + distance + ') | <b>DURATION:</b> ' + durH + 'h ' + durM.toString().padStart(2,'0') + 'm | <b>MEAL:</b> Included<br><b>IMPORTANT:</b> Present this boarding pass with valid ID at check-in counter 2 hours before departure. Boarding closes 45 mins before.</div></div><div class="qr-box">' + qrHtml + '<div style="font-size:10px;font-weight:800;margin-top:8px">SCAN TO TRACK LIVE FLIGHT STATUS</div><div class="qr-link">' + trackLink + '</div><button class="no-capture" onclick="copyRobust(\'' + trackLink + '\')" style="margin-top:6px;background:#1a2b5e;color:#fff;border:none;padding:4px 8px;border-radius:4px;font-size:9px;font-weight:800;cursor:pointer">COPY LINK</button></div></div><div class="bottom-bar">This is an official e-ticket issued by SKYLINK AIRLINES. Non-transferable. Subject to conditions of carriage.</div><div class="stub">BOARDING PASS STUB - KEEP WITH YOU<br>' + b.name + ' | ' + b.flight + ' | ' + b.from + ' ' + b.to + ' | SEAT ' + b.seat + ' | GATE ' + b.gate + ' | TERMINAL ' + b.terminal + ' | ' + b.tracking + '</div></div></div><div class="btn-area"><button class="btn-dl" onclick="downloadHD()">Download Bright HD</button><button class="btn-tr" onclick="location.href=\'/track?code=' + b.tracking + '\'">Live Track</button><button class="btn-cp no-capture" onclick="copyRobust(\'' + b.tracking + '\')">Copy Code</button></div><div id="msg" style="font-size:12px;font-weight:800;color:#16a34a;text-align:center;margin:8px;display:none"></div><script>function copyRobust(t){try{if(navigator.clipboard && window.isSecureContext){navigator.clipboard.writeText(t).then(()=>showMsg("Copied: "+t)).catch(()=>fallback(t))}else{fallback(t)}}catch(e){fallback(t)}}function fallback(t){const ta=document.createElement("textarea");ta.value=t;ta.style.position="fixed";ta.style.left="-9999px";document.body.appendChild(ta);ta.select();try{document.execCommand("copy");showMsg("Copied: "+t)}catch(e){showMsg(t)}document.body.removeChild(ta)}function showMsg(m){const el=document.getElementById("msg");el.innerText=m;el.style.display="block";setTimeout(()=>el.style.display="none",3000)}function downloadHD(){const btns=document.querySelectorAll(".no-capture");btns.forEach(b=>b.style.display="none");const el=document.getElementById("ticketCapture");showMsg("Generating HD...");html2canvas(el,{scale:3,backgroundColor:"#ffffff",useCORS:true}).then(canvas=>{const link=document.createElement("a");link.download="SKYLINK-' + b.booking + '-' + b.tracking + '-HD.png";link.href=canvas.toDataURL("image/png",1.0);link.click();showMsg("Saved to Gallery - Bright HD - Official Clean");btns.forEach(b=>b.style.display="")}).catch(()=>{window.print();btns.forEach(b=>b.style.display="")})}<\/script></body></html>');
});
app.get('/track', async (req,res)=>{
  ...
  res.send('<html><head><meta name="viewport" content="width=device-width,initial-scale=1"><title>Live Flight Tracking ' + b.tracking + '</title><style>body{margin:0;background:#fff;font-family:Arial,Helvetica,sans-serif;padding:16px;color:#111}.container{max-width:700px}h2{margin:0 0 16px;font-size:18px;font-weight:700;display:flex;align-items:center;gap:6px}.line{margin:8px 0;font-size:14px}.label{font-weight:700}.value{font-weight:400}.divider{border:none;border-top:1.5px solid #1a2b5e;margin:16px 0}.live{margin-top:10px}.status-box{padding:6px 10px;border-radius:6px;font-weight:900;font-size:13px;display:inline-block}</style></head><body><div class="container"><h2>📍 Live Flight Tracking</h2><div class="line"><span class="label">Tracking Code:</span> <span class="value">' + b.tracking + '</span></div><div class="line"><span class="label">Passenger:</span> <span class="value">' + b.name + '</span></div><div class="line"><span class="label">Flight:</span> <span class="value">' + b.flight + '</span></div><div class="line"><span class="label">Route:</span> <span class="value">' + b.from + ' → ' + b.to + ' (' + b.fromFull + ' to ' + b.toFull + ')</span></div><div class="line"><span class="label">Departure:</span> <span class="value">' + departStr + ' (' + b.fromTz + ')</span></div><div class="line"><span class="label">Est. Duration:</span> <span class="value">' + totalH + 'h ' + totalM + 'm' + (b.distanceKm? ' | ' + b.distanceKm + 'km' : '') + '</span></div><div class="line"><span class="label">Est. Arrival:</span> <span class="value">' + arriveStr + ' (' + b.toTz + ')</span></div><div class="line"><span class="label">Aircraft:</span> <span class="value">' + (b.aircraft||'Boeing 737-800') + '</span></div><hr class="divider"><div class="live"><div style="font-weight:700;margin-bottom:8px">Live Status - Updates Every Second - 100% Real</div><div class="line"><span class="label">Status:</span> <span id="status" class="status-box">Loading...</span></div><div class="line"><span class="label">Time in Air:</span> <span id="timeInAir" class="value" style="font-weight:900;font-size:16px">Calculating...</span></div><div class="line"><span class="label">Altitude:</span> <span id="alt" class="value">N/A</span></div><div class="line"><span class="label">Speed:</span> <span id="spd" class="value">N/A</span></div><div class="line" style="margin-top:12px;font-size:12px;color:#64748b"><span id="countdown"></span></div></div></div><script>const departISO="' + departISO + '";const arriveISO="' + arriveISO + '";const departMs=new Date(departISO).getTime();const arriveMs=new Date(arriveISO).getTime();const totalMs=arriveMs-departMs;function updateLive(){const now=Date.now();const diff=now-departMs;const remain=arriveMs-now;const statusEl=document.getElementById("status");const timeEl=document.getElementById("timeInAir");const altEl=document.getElementById("alt");const spdEl=document.getElementById("spd");const cdEl=document.getElementById("countdown");if(diff<0){const abs=Math.abs(diff);const hrs=Math.floor(abs/3600000);const mins=Math.floor((abs%3600000)/60000);const secs=Math.floor((abs%60000)/1000);statusEl.innerText="Scheduled - Not Yet Departed";statusEl.style.background="#dbeafe";statusEl.style.color="#1e40af";timeEl.innerText="Not Departed";timeEl.style.color="#1e40af";altEl.innerText="0 ft (On Ground)";spdEl.innerText="0 km/h";cdEl.innerText="Departs in "+hrs+"h "+mins+"m "+secs+"s - Exact time: "+new Date(departISO).toLocaleString()}else if(diff>=totalMs){statusEl.innerText="Arrived ✅ Landed";statusEl.style.background="#dcfce7";statusEl.style.color="#166534";const th=Math.floor(totalMs/3600000);const tm=Math.floor((totalMs%3600000)/60000);timeEl.innerText=th+"h "+tm+"m (Flight Completed)";timeEl.style.color="#16a34a";altEl.innerText="0 ft (Landed)";spdEl.innerText="0 km/h";cdEl.innerText="Flight completed at "+new Date(arriveISO).toLocaleString()+". Total: "+th+"h "+tm+"m"}else{const h=Math.floor(diff/3600000);const m=Math.floor((diff%3600000)/60000);const s=Math.floor((diff%60000)/1000);let stat="In-Flight ✈️";let alt=0;let spd=0;if(diff<5*60000){stat="Boarding / Taxiing";alt=0;spd=25}else if(diff<15*60000){stat="Departed - Climbing";const prog=(diff-5*60000)/(10*60000);alt=Math.floor(prog*35000);spd=Math.floor(250+prog*300)}else if(diff>totalMs-20*60000){stat="Descending for Landing";const prog=(totalMs-diff)/(20*60000);alt=Math.floor(prog*35000);spd=Math.floor(300+prog*400)}else{stat="Cruising - In Flight ✈️";alt=35000;spd=880}statusEl.innerText=stat;statusEl.style.background="#dcfce7";statusEl.style.color="#166534";timeEl.innerText=h+"h "+m+"m "+s+"s";timeEl.style.color="#16a34a";altEl.innerText=alt.toLocaleString()+" ft";spdEl.innerText=spd+" km/h";const rh=Math.floor(remain/3600000);const rm=Math.floor((remain%3600000)/60000);const rs=Math.floor((remain%60000)/1000);cdEl.innerText="Live - "+rh+"h "+rm+"m "+rs+"s remaining - Landing: "+new Date(arriveISO).toLocaleString()}}updateLive();setInterval(updateLive,1000);<\/script></body></html>');
});
app.get('/health',(req,res)=> res.send('OK'));
app.listen(PORT, ()=> console.log('SKYLINK FINAL V3 REAL BUSINESS - 100% REAL DYNAMIC AIRCRAFT'));
