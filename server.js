// server.js - SKYLINK V6 HORIZONTAL LOCK OFFICIAL - V5 BASE + HORIZONTAL FIX ONLY
...
// === V6 HORIZONTAL LOCK - ONLY CHANGE FROM V5 IS HERE - EXACTLY LIKE YOUR PDF ===
app.get('/boarding-pass', async (req,res)=>{
  ...
  // HORIZONTAL LOCK: viewport=1024 forces desktop layout on mobile, no media query that stacks
  res.send('<html><head><meta name="viewport" content="width=1024"><title>Boarding Pass ' + b.booking + '</title><script src="https://cdnjs.cloudflare.com/ajax/libs/html2canvas/1.4.1/html2canvas.min.js"><\/script><style>*{box-sizing:border-box} html,body{margin:0;padding:0;background:#e5e7eb;font-family:Arial,Helvetica,sans-serif;overflow-x:auto}.page-center{width:100%;min-width:1024px;display:flex;justify-content:center;background:#e5e7eb;padding:0}.ticket-wrap{width:900px;min-width:900px}.ticket{width:900px;min-width:900px;max-width:900px;background:#fff;margin:0;box-shadow:0 4px 20px rgba(0,0,0,.15);filter:brightness(1.15)}.header{background:#1a2b5e;color:#fff;padding:18px 24px;display:flex;justify-content:space-between;align-items:flex-start}.header h1{margin:0;font-size:28px;font-weight:900;letter-spacing:0.5px}.header h1 span{color:#facc15}.header.sub{font-size:10px;letter-spacing:1px;margin-top:4px;opacity:0.9}.header-right{font-size:11px;opacity:0.8;text-align:right}.content{padding:20px 24px;display:grid;grid-template-columns:1fr 200px;gap:20px;width:900px;min-width:900px}.label{font-size:10px;color:#6b7280;font-weight:700;letter-spacing:0.5px;text-transform:uppercase;margin-top:14px}.value{font-size:14px;font-weight:800;color:#111827;margin-top:2px;word-break:break-word}.big-name{font-size:18px;font-weight:900}.grid3{display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin-top:6px}.status-green{color:#16a34a;font-weight:900}.qr-box{border:1.5px solid #1a2b5e;padding:10px;text-align:center;background:#fff;height:fit-content}.qr-box img{width:100%;max-width:180px;height:auto}.qr-link{font-size:8px;color:#1a2b5e;word-break:break-all;margin-top:6px;font-weight:600}.bottom-bar{background:#1a2b5e;color:#cbd5e1;padding:8px 24px;font-size:8px;text-align:center;letter-spacing:0.5px;width:900px;min-width:900px}.stub{padding:10px 24px;background:#fff;border-top:2px dashed #9ca3af;font-size:10px;font-weight:800;width:900px;min-width:900px}.btn-area{padding:14px;display:flex;gap:8px;justify-content:center;background:#f1f5f9;width:900px;min-width:900px}.btn-dl{background:#16a34a;color:#fff;border:none;padding:12px 18px;border-radius:8px;font-weight:900;cursor:pointer}.btn-tr{background:#0f2e6d;color:#fff;border:none;padding:12px 18px;border-radius:8px;font-weight:900;cursor:pointer}.btn-cp{background:#fff;color:#0f2e6d;border:1.5px solid #0f2e6d;padding:12px 18px;border-radius:8px;font-weight:900;cursor:pointer}.no-capture{} /* REMOVED @media that caused vertical on mobile */ </style></head><body><div class="page-center"><div class="ticket-wrap" id="ticketCapture"><div class="ticket"><div class="header"><div><h1>SKYLINK<span>AIRLINES</span></h1><div class="sub">IATA CERTIFIED • EST. 2018 • OFFICIAL BOARDING PASS</div></div><div class="header-right">' + host + '<br>' + b.tracking + '</div></div><div class="content"><div><div class="label">PASSENGER NAME / NOM DU PASSAGER</div><div class="value big-name">' + b.name + '</div><div class="label">FROM / DE</div><div class="value">' + b.fromFull + '</div><div class="label">TO / A</div><div class="value">' + b.toFull + '</div><div class="grid3"><div><div class="label">FLIGHT / VOL</div><div class="value">' + b.flight + '</div></div><div><div class="label">DATE</div><div class="value">' + new Date(b.departISO).toLocaleDateString('en-GB') + '</div></div><div><div class="label">SEAT / SIEGE</div><div class="value" style="font-size:18px">' + b.seat + '</div></div></div><div class="grid3"><div><div class="label">GATE / PORTE</div><div class="value">' + b.gate + '</div></div><div><div class="label">TERMINAL</div><div class="value">' + b.terminal + '</div></div><div><div class="label">CLASS</div><div class="value">' + b.class + '</div></div><div><div class="label">BAGGAGE</div><div class="value">' + b.baggage + '</div></div></div><div class="grid3" style="margin-top:12px"><div><div class="label">TRACKING CODE</div><div class="value">' + b.tracking + '</div></div><div><div class="label">STATUS</div><div class="value status-green">CONFIRMED / CONFIRME</div></div></div><div style="margin-top:16px"><div class="label">DEPARTURE / DEPART</div><div class="value">' + departStr + '</div><div class="label">ARRIVAL / ARRIVEE</div><div class="value">' + arriveStr + '</div></div><div style="margin-top:16px;font-size:11px;line-height:1.5"><b>AIRCRAFT:</b> ' + aircraft + ' (' + distance + ') | <b>DURATION:</b> ' + durH + 'h ' + durM.toString().padStart(2,'0') + 'm | <b>MEAL:</b> Included<br><b>IMPORTANT:</b> Present this boarding pass with valid ID at check-in counter 2 hours before departure. Boarding closes 45 mins before.</div></div><div class="qr-box">' + qrHtml + '<div style="font-size:10px;font-weight:800;margin-top:8px">SCAN TO TRACK LIVE FLIGHT STATUS</div><div class="qr-link">' + trackLink + '</div><button class="no-capture" onclick="copyRobust(\'' + trackLink + '\')" style="margin-top:6px;background:#1a2b5e;color:#fff;border:none;padding:4px 8px;border-radius:4px;font-size:9px;font-weight:800;cursor:pointer">COPY LINK</button></div></div><div class="bottom-bar">This is an official e-ticket issued by SKYLINK AIRLINES. Non-transferable. Subject to conditions of carriage.</div><div class="stub">BOARDING PASS STUB - KEEP WITH YOU<br>' + b.name + ' | ' + b.flight + ' | ' + b.from + ' ' + b.to + ' | SEAT ' + b.seat + ' | GATE ' + b.gate + ' | TERMINAL ' + b.terminal + ' | ' + b.tracking + '</div></div></div></div><div style="width:100%;min-width:1024px;display:flex;justify-content:center"><div class="btn-area"><button class="btn-dl" onclick="downloadHD()">Download Bright HD</button><button class="btn-tr" onclick="location.href=\'/track?code=' + b.tracking + '\'">Live Track</button><button class="btn-cp no-capture" onclick="copyRobust(\'' + b.tracking + '\')">Copy Code</button></div></div><div id="msg" style="font-size:12px;font-weight:800;color:#16a34a;text-align:center;margin:8px;display:none"></div><script>function copyRobust(t){try{if(navigator.clipboard && window.isSecureContext){navigator.clipboard.writeText(t).then(()=>showMsg("Copied: "+t)).catch(()=>fallback(t))}else{fallback(t)}}catch(e){fallback(t)}}function fallback(t){const ta=document.createElement("textarea");ta.value=t;ta.style.position="fixed";ta.style.left="-9999px";document.body.appendChild(ta);ta.select();try{document.execCommand("copy");showMsg("Copied: "+t)}catch(e){showMsg(t)}document.body.removeChild(ta)}function showMsg(m){const el=document.getElementById("msg");el.innerText=m;el.style.display="block";setTimeout(()=>el.style.display="none",3000)}function downloadHD(){const btns=document.querySelectorAll(".no-capture");btns.forEach(b=>b.style.display="none");const el=document.getElementById("ticketCapture");showMsg("Generating HD...");html2canvas(el,{scale:3,backgroundColor:"#ffffff",useCORS:true,width:900,windowWidth:1024}).then(canvas=>{const link=document.createElement("a");link.download="SKYLINK-' + b.booking + '-' + b.tracking + '-HD.png";link.href=canvas.toDataURL("image/png",1.0);link.click();showMsg("Saved to Gallery - Bright HD - Official Clean");btns.forEach(b=>b.style.display="")}).catch(()=>{window.print();btns.forEach(b=>b.style.display="")})}<\/script></body></html>');
});
...
app.listen(PORT, ()=> console.log('SKYLINK V6 HORIZONTAL LOCK - OFFICIAL - READY'));
